...
2. [`/schema/diff`](#retrieve-schema-difference): Compare the snapshot with the schema of another instance (e.g. fresh instance or instance in a different environment) and retrieve a `diff` containing all differences
3. [`/schema/apply`](#apply-schema-difference): Update the schema of another instance based on the previously obtained diff
...
This endpoint does not allow different Directus versions and database vendors by default. This is to avoid any unintentional
